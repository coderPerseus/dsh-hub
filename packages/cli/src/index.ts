...
function parseArgs(argv: string[]): Parsed {
  const parsed: Parsed = { command: argv[0], positional: [], values: new Map(), flags: new Set() };
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (!valueOptions.has(option)) throw new Error(`Unknown option: ${arg}`);
...
function last(parsed: Parsed, name: string): string | undefined {
  return parsed.values.get(name)?.at(-1);
}

function textSearch(result: SearchPluginsResult): string {
  if (result.items.length === 0) return "No plugins found.";
...
function textPlugin(plugin: PluginDetail): string {
  return [
...
export async function runCli(argv: string[], io: Io, providedClient?: Client): Promise<number> {
  try {
    ...
    if (parsed.command === "search") {
      ...
      const compatibility = (parsed.values.get("compatibility") ?? []) as CompatibilityStatus[];
      ...
      const sort = last(parsed, "sort") as SearchPluginsInput["sort"];
      ...
      const result = await client.search({
        ...
        limit,
        sort,
        locale: last(parsed, "locale") as SearchPluginsInput["locale"],
      });
    ...
    if (parsed.command === "plugin") {
      if (parsed.positional.length !== 1) throw new Error("plugin requires one owner/repository slug");
      const plugin = await client.plugin(
        parsed.positional[0],
        last(parsed, "locale") as SearchPluginsInput["locale"],
      );
      if (plugin === null) {
