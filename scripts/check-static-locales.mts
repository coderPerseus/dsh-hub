...
for(const locale of locales){
  ...
  for(const page of ['/',...slugs.map(s=>'/plugins/'+s)]){
    ...
    assert(head.includes(`<link rel="canonical" href="https://dshhub.org${escape(url)}">`),url);
    for(const language of locales)assert(head.includes(`hreflang="${language}" href="https://dshhub.org${escape(localizedHref(page,language))}"`),url);
  ...
  assert.equal((sitemap.match(/<loc>/g)||[]).length,slugs.length+1);
  ...
  console.log(`Verified ${locale}: ${slugs.length+1} pages, metadata, sitemap, structured data and 404`);
}
