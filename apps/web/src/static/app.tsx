...
import type { StaticIndex } from '../../../../packages/client/src/static';
...
export type PageData = {locale: Locale; manifest: WebManifest; listing?: ListingPage; variants?: Array<{slug: string; repository: string}>; canonicalSlug?: string; initial: StaticIndex; plugin?: CatalogPlugin; related: PluginSummary[]; notFound?: boolean};
export function App({data}: {data: PageData}) {
  ...
  const pagePath = data.plugin ? '/plugins/' + data.plugin.slug : data.listing?.path || '/';
